...
const Navbar = () => {
  ...
  const clearAttendanceData = async () => {
    if (!window.confirm("คุณแน่ใจหรือไม่ว่าต้องการล้างข้อมูลการเช็คชื่อทั้งหมด?")) {
      return;
    }
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.delete("http://localhost:3000/api/attendance", {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
